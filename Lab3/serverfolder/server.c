...
// function to update the window, which is an array that holds all the received packets
// the function will also be used to update the acknowledgement buffer (array of packets already acknowledged)
int updatewindow(bool window[], int number_of_packets, int packet_to_add) {
    // this function attempts to update the window
    // returns 1 if the window updated successfully, returns 0 for some other error, returns 2 for already added
    if (window[packet_to_add-1] == true) {
    // packet already received
        return 2;
    }
    else {
        window[packet_to_add-1] = true;
        return 1;
    }
    return 0;
}
...
int main(int argc, char const *argv[])
{
    ...
    while (currently_receiving) {
        ...
        if (frag_no == 1) {
            ...
            received_packets = malloc(total_frag*sizeof(bool));
			int i; 
			for (i = 0; i < total_frag; i++) {
				received_packets[i] = false; 			
			}  

            // update the receive window, if we already received the packet, ignore it
            if (updatewindow(received_packets,total_frag,frag_no) == 2) {
				printf("Duplicate Packet!, Ignored last Packet Sent.");					
				continue;
			}
           ...
           if (updatewindow(received_packets,total_frag,frag_no)==2) continue;
