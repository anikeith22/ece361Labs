...
struct account_struct* new_account_char_status(struct account_struct** list, char new_cID[], char new_cPW[], int * status) {
...
int register_helper(struct account_struct** list, char new_cID[], char new_cPW[]) {
    int status = 0;
    new_account_char_status(list, new_cID, new_cPW, &status);
    return status;
}
...
void store_user_list(struct account_struct* list, char store_to[]) {
    if (list == NULL) {
  ...
  int user_no = 0;
  while (root != NULL) {
    if (user_no > 0) strcat(store_to, ",");
    user_no++;
    strcat(store_to, root->cID);
...
int get_user_session(struct account_struct* list, char user[]) {
  if (list == NULL) return false;
  ...
  while (curr_account != NULL) {
    if (strcmp(curr_account->cID,user)==0) {
      connected = curr_account->inSession;
      return curr_account->connectedToSession;
      break;
    }
    curr_account = curr_account->next_user;
  } 
...
void print_user_list(struct account_struct* list) {
  ...
  while (root != NULL) {
    user_no++;
    printf("User Number: %d, User Name: %s, Password: %s, Logged In: %d, Connected: %d, Session: %d, Socket: %d\n", user_no, root->cID, root->cPW, root->loggedIn, root->inSession, root->connectedToSession, root->socket);
    root = root->next_user;
  }
...
int user_login(struct account_struct **list, char user[], char pass[], int client_socket) {
...
bool add_user_to_session(struct session_struct **session, struct account_struct *account, int sID){
    ...
    if (temp) {
      ...
      if ((*session)->connected_users == NULL) new_account(&(*session)->connected_users, account);
      else new_account(&(*session)->connected_users->next_user, account);
      (*session)->userCount++;
...
//session_list[0]->connected_users->cID;
...
//    new_account_char(&account_list, "anikeith2", "test");
//    printf("TEST\n\n");
//    int status = 0;
//    printf("%d\n",register_helper(&account_list, "jay","test"));
//    printf("\n");
//    print_user_list(account_list);
//    printf("TEST2\n\n");
//    printf("%d\n",register_helper(&account_list, "jay222","test"));
//    printf("\n");
//    print_user_list(account_list);
//
//
